...
namespace WindowsFormsApp56
{
    class BusinessRecognition
    {
        ...
        #region Veri Kaydet
        public bool SaveTrainingData(Image face_data, string FaceName, string cocuk, string combo)
        {
            try
            {
                ...
                if (File.Exists(Dizin + XmlVeriDosyasi))
                {
                    ...
                    XmlElement role_D = docu.CreateElement("ROLE");
                    ...
                    if (combo.Equals("Baba"))
                    {
                        NAME_PERSON = NAME_PERSON + "(" + cocuk + "-" + combo + "si" + ")";
                    }
                    if (combo.Equals("Anne"))
                    {
                        NAME_PERSON = NAME_PERSON + "(" + cocuk + "-" + combo + "si" + ")";
                    }
                    if (combo.Equals("Kardes"))
                    {
                        NAME_PERSON = NAME_PERSON + "(" + cocuk + "-" + combo + "i" + ")";
                    }
                    if (combo.Equals("Buyukanne"))
                    {
                        NAME_PERSON = NAME_PERSON + "(" + cocuk + "-" + combo + "si" + ")";
                    }
                    if (combo.Equals("Buyukbaba"))
                    {
                        NAME_PERSON = NAME_PERSON + "(" + cocuk + "-" + combo + "si" + ")";
                    }
                    if (combo.Equals("Komsu"))
                    {
                        NAME_PERSON = NAME_PERSON + "(" + cocuk + "-" + combo + "su" + ")";
                    }
                    name_D.InnerText = NAME_PERSON;
                    role_D.InnerText = combo;
                ...
                else
                {
                    ...
                    using (XmlWriter writer = XmlWriter.Create(FS_Face))
                    {
                        ...
                        writer.WriteElementString("ROLE", combo);
